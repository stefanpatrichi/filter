...
void BMPImage::blur(const int radius) {
  ...
  for (int l = 0; l < height; ++l) {
    ...
    for (int first = 1, last = 2 * radius + 1; first <= width - 2 * radius - 1; ++first, ++last) {
      ...
      aux[l][first + radius + 1] = sum / radius;
    }
  }
  ...
  for (int c = 0; c < width; ++c) {
    ...
    for (int l = 0; l < 2 * radius + 1; ++l) 
      sum += data[l][c];
    aux[1][c + 1] += sum;
    aux[2 * radius + 1 + 1][c + 1] -= sum;

    // next windows
    for (int first = 1, last = 1 + 2 * radius + 1 - 1; first <= height - 2 * radius - 1; ++first, ++last) {
      sum += data[last][c];
      sum -= data[first - 1][c];
      aux[first + 1][c + 1] += sum;
      aux[last + 1 + 1][c + 1] -= sum;
    }
  }

  // build blurred image from difference 2d array aux
  for (int l = 1; l <= height; ++l) 
    for (int c = 1; c <= width; ++c)
      data[l - 1][c - 1] = btor(aux[l - 1][c] + aux[l][c - 1] - aux[l - 1][c - 1] + data[l - 1][c - 1]);
}
